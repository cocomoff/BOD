...
from sample import generate_sample_undirected
from typing import Dict, Tuple
from heapq import heappop, heappush


def BOAstar(
    G: nx.Graph, start: int, goal: int, h: Dict[int, Tuple[int, int]]
...
if __name__ == "__main__":
    ...
    dist_c1 = nx.single_source_dijkstra_path_length(G, goal, weight="c1")
    dist_c2 = nx.single_source_dijkstra_path_length(G, goal, weight="c2")
    for n in G.nodes():
        if n not in dist_c1:
            dist_c1[n] = float("inf")
        if n not in dist_c2:
